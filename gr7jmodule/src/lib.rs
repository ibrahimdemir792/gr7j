...
#[pyfunction]
#[pyo3(name = "gr7j")]
fn gr7j_py(parameters: &PyList, rainfall: &PyArray1<f64>, evapotranspiration: &PyArray1<f64>, states: &PyArray1<f64>, uh1: &PyArray1<f64>, uh2: &PyArray1<f64>, flow: &PyArray1<f64>) {
    ...
    let mut n_uh2 = unsafe { uh2.as_array_mut() };
    let mut n_flow = unsafe { flow.as_array_mut() };
    gr7j::gr7j(&v_param, &mut n_rainfall, &mut n_evap, &mut n_states, &mut n_uh1, &mut n_uh2, &mut n_flow);
}
